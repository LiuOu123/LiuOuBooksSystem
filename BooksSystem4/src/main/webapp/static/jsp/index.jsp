...
<center>
    <h2>图书借阅系统</h2>
    <form action="/tiaocha" method="post">
        <a>图书分类:</a>
        <select name="lei">
...
<div>
    <a href="/chushi?pageNum=${pageInfo.firstPage}">首页</a>
<c:if test="${pageInfo.hasPreviousPage}">
    <a href="/chushi?pageNum=${pageInfo.prePage}">上一页</a>
</c:if>
<c:forEach items="${pageInfo.navigatepageNums}" var="i">
    <a href="/chushi?pageNum=${i}">${i}</a>
</c:forEach>
<c:if test="${pageInfo.hasNextPage}">
    <a href="/chushi?pageNum=${pageInfo.nextPage}">下一页</a>
</c:if>
        <a href="/chushi?pageNum=${pageInfo.lastPage}">尾页</a>
</div>
</center>
</body>
</html>
<script type="text/javascript">
        ...
        function tui() {
               ...
               if(flag){
                   location.href="/static/jsp/login.jsp";
               }
               return;
        }
</script>
